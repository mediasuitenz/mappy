...
class KeyController {
  ...
  /**
   * Find out whether a layer is in the key
   * Key visibility map has all defined key layers added to it, so we can
   * leverage this to see if the given layer has been added
   */
  hasLayer(name) {
    return typeof this.keyVisibility_[name] !== 'undefined'
  }
...
module.exports = (config) => {
  ...
  assert.ok(config.key, '`key` must be provided')
