...
function addKey(name, description, visible) {
  key.addItem(name, description, visible)
}
...
function geojsonLayer(layerConfig) {
  ...
  layerVisibility[layerConfig.name] = {}
  layerVisibility[layerConfig.name].isVisible = layerConfig.startVisible
  ...
  dataService.on('data', () => {
    ...
    if (layerVisibility[layerConfig.name].isVisible) map.showGeojsonLayer(layerConfig.name)
...
layerConfigs.forEach((layerConfig) => {
  if (layerConfig.showInKey)
    addKey(layerConfig.name, layerConfig.description, layerVisibility[layerConfig.name].isVisible)
})

//listen for user to toggle a key item on or off
key.on('layerToggle', (event) => {
  layerVisibility[event.key].isVisible = event.checked
  if (event.checked) map.showGeojsonLayer(event.key)
  else map.hideGeojsonLayer(event.key)
})
